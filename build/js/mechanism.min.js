  ...
  var slider = {
    ...
    setUserOptions: function setUserOptions() {
      ...
      var defaults = {
        ...
        delay: 5000,
        // delay between transitions in ms {number}
        enableDrag: true,
        // enable drag option {boolean}
        autoplay: false,
        // slider autoplay {boolean}
        slidesPerView: {},
        // number of slides per view {object}
    ...
    setSliderOptions: function setSliderOptions() {
      ...
      var _this$options = this.options,
          ...
          pagination = _this$options$class.pagination; // DOM elements
      ...
      this.pagination = this.container.querySelector(".".concat(pagination)); // Options

      this.disableEvents = false;
      this.index = 1;
      this.slidesPerView = 1;
      this.maxSlidesPerView = Math.max.apply(Math, _toConsumableArray(Object.keys(slidesPerView).map(function (key) {
    ...
    init: function init() {
      // Set options
      this.setUserOptions();
      this.setSliderOptions(); // Create slides and set wrapper

      this.calculateSlidesPerView();
      this.createClones();
      this.setWidth();
      this.moveWrapper(); // Pagination

      this.createPagination(); // Autoplay

      this.autoplay(); // Buttons
    ...
    attachEvents: function attachEvents() {
      ...
      if (enableDrag) {
        ...
        this.container.addEventListener('touchend', this.touchendHandler.bind(this)); // Mouse
    ...
    createClones: function createClones() {
      ...
      for (var i = 0; i < this.maxSlidesPerView; i++) {
        if (slidesLength - i < 0 || i > slidesLength) break; // Copy the slides from the end

        cloned = wrapper.children[slidesLength - i].cloneNode(true);
        clonesAtBack.appendChild(cloned); // Copy the slides from the beginning
    ...
    setWidth: function setWidth() {
      ...
      Object.keys(this.allSlides).map(function (index) {
        var slide = _this2.allSlides[index]; // Slide width

        slide.style.width = slideWidth; // Wrapper width

        _this2.wrapperWidth += slide.offsetWidth; // Maximum drag offset
    ...
    changeSlide: function changeSlide(direction) {
      ...
      if (!this.disableEvents) {
        ...
        direction == 'right' ? this.index++ : this.index--; // Disable events during slider animation

        this.disableEvents = true; // Highlight bullet
        ...
        setTimeout(function () {
          ...
          onChange(_this3.slides[_this3.index - 1]); // Enable Events
    ...
    createPagination: function createPagination() {
      ...
      var bullet; // Create bullets

      for (var i = 0; i < slidesLength; i++) {
        bullet = document.createElement('span');
        bullet.classList.add(paginationItem); // Add active class to the first bullet
      ...
      this.pagination.appendChild(fragment); // Bullets action
    ...
    highlightBullet: function highlightBullet() {
      if (!this.pagination) return;
      var paginationItem = this.options["class"].paginationItem; // Remove active class from bullet

      var activeBullet = this.pagination.querySelector('.is-active');
      activeBullet.classList.remove('is-active'); // Add class to active bullet
    ...
    updateSliderAfterDrag: function updateSliderAfterDrag() {
      ...
      if (!this.drag.dragDiff) return; // Autoplay

      this.autoplay(); // Move slider
